...
export default function NotificationsBell(){
  ...
  async function load(){
    try{ const { data } = await listNotifications(token); setItems(data.items); }catch{}
  }
          ...
          <div style={{display:'grid', gap:10, maxHeight:260, overflow:'auto'}}>
            {items.map(n=>(
              <div key={n._id} className="helper" style={{borderBottom:'1px solid #1f2a44', paddingBottom:8}}>
                {n.type==='post_resolved' ? 'Tu incidencia fue RESUELTA' : 'Tu incidencia está EN PROGRESO'}
                <br/>ID: {n.payload?.postId} • {new Date(n.createdAt).toLocaleString()}
              </div>
            ))}
